...
Listado.prototype.buscarRestaurante = function(id) {
    //restArr = this.restaurantes
    
    const found = this.restaurantes.find(el => el.id === id);
    if (found) {
...
Listado.prototype.obtenerUbicaciones = function() {
    ...
    ciudadesRepetidas = this.restaurantes.map(function(restaurant){
        return restaurant.ubicacion;
    })
    return (this.eliminaElementosRepetidos(ciudadesRepetidas)).sort();
}
//CODIGO VIEJO
...
Listado.prototype.obtenerRubros = function() {
    var rubrosRepetidos = [];
    rubrosRepetidos = this.restaurantes.map(function(restaurante){
        return restaurante.rubro;
    })
    
    return (this.eliminaElementosRepetidos(rubrosRepetidos)).sort();
}
    /*CODIGO VIEJO
    for (var i = 0; i < this.restaurantes.length; i++) {
...
Listado.prototype.obtenerHorarios = function() {
    ...
    var horariosObtenidos = [];
    //Recorremos el array de restaurantes y vamos agregando todos los array de horarios
    horariosObtenidos = this.restaurantes.reduce(function(acum, actual){
        return acum.concat(actual.horarios
            );
    },[]);

    return (this.eliminaElementosRepetidos(horariosObtenidos)).sort();
}
    /* CODIGO VIEJO
    //En este arreglo vamos a poner todos los horarios, uno por uno
    var horarioUnoPorUno = [];
    horariosObtenidos.forEach(function(a) {
        a.forEach(function(hor) {
            horarioUnoPorUno.push(hor)
        });
    });

    //En este arreglo vamos a poner todos los horarios pero sin repetidos
    for (var i = 0; i < this.restaurantes.length; i++) {
        horariosObtenidos.push(this.restaurantes[i].horarios);
    }
    var horarioSinRepetir = horarioUnoPorUno.filter(function(elem, index, self) {
        return index === self.indexOf(elem);
    });

    return horarioSinRepetir.sort();
    *****************************************************/


Listado.prototype.eliminaElementosRepetidos = function(elementosFIltrar){
    var elementosRepetidos = elementosFIltrar.filter(function(elem, index, self) {
        return index === self.indexOf(elem);
    });
    return elementosRepetidos.sort();
}
